<script lang="ts" defer>
	...
	const selectFile = function (input: File) {
		currentFile = input;
		closeModal();
	};
	...
	let { display = $bindable(false), currentFile = $bindable(undefined) }: Props = $props();

	export const path: string = 'private/test assets';
...
<div
	id="popup-modal"
	class="popup-modal absolute bottom-0 left-0 right-0 top-0 z-10 p-32 opacity-0 hidden"
    class:!grid={display}
>
	<div
		id="popup-modal-helper"
		class="relative overflow-hidden mx-auto my-auto flex min-h-48 w-full flex-col rounded-lg border border-neutral-800 bg-neutral-950 shadow-lg transition-transform"
	>
		<div
			class="relative flex min-h-[4.5rem] w-full items-center gap-2 border-b border-neutral-800 px-4"
		>
			<div
				class="absolute right-20 top-3 z-10 flex w-fit rounded-full border border-neutral-800 bg-neutral-950 px-1 py-1 text-neutral-500 shadow-lg"
			>
				<button
			...
			<div
				class="absolute right-5 top-3 z-10 flex w-fit rounded-full border border-neutral-800 bg-neutral-950 px-1 py-1 text-neutral-500 shadow-lg"
			>
				<button
			...
			<nav class="my-auto flex gap-2">
				<div class="flex rounded-full border border-neutral-800 p-1">
					<button
						onmouseenter={() => hoverOver('create-layout')}
						onmouseleave={() => hoverOver(undefined)}
						class="flex h-10 w-10 items-center justify-center rounded-full text-neutral-500 hover:bg-svelted-gray-700 hover:text-white"
					>
					...
					<button
						...
						onmouseleave={() => hoverOver(undefined)}
						class="flex h-10 w-10 items-center justify-center rounded-full text-neutral-500 hover:bg-red-500 hover:text-white"
					>
					...
					<div class="flex-grow">
						<button
							...
							onmouseleave={() => hoverOver(undefined)}
							class="contens absolute rounded-l-full text-neutral-500 hover:text-white focus:bg-[#2da05a] focus:text-white focus:outline-none"
						>
							{#if client.hoverOver == 'filter'}
								<Funnel
									class="h-10 w-11 rounded-l-full border-r border-neutral-950 fill-[currentcolor] p-2 px-2.5 hover:bg-[#278c4c]"
									weight="fill"
								/>
							{:else}
								<Funnel
						...
						<input
							...
							type="text"
							class="w-full rounded-full bg-[#161616] px-3 py-2 pl-14 text-neutral-300 focus:outline-none focus:ring-1 focus:ring-[#36bf68]"
						/>
					</div>
				</div>
			</nav>
		...
		<div class="flex bg-neutral-950">
			...
			<div class="h-full-editor relative flex w-full flex-col px-3 text-white">
				<div class="max-h-[75vh] flex-grow overflow-y-scroll pt-4">
							...
							<table class="w-full pb-1">
								<tbody class="text-neutral-500">
									{#each folders as folder, index (folder)}
										<tr class=" hover:text-white" animate:flip={{ duration: 500 }}>
											<td class="rounded-md !bg-neutral-950 !p-0 px-2 hover:!bg-[#0a2620]">
												<a
					...
					{:else}
						...
						<div class="rounded-lg bg-svelted-gray-700 px-2">
							<table class="files mb-2 w-full pb-1">
								<thead>
...
<style>
	...
	.files tr td:first-of-type {
		border-top-left-radius: 0.5rem;
		border-bottom-left-radius: 0.5rem;
	}

	.files tr td:nth-last-of-type(2) {
		border-top-right-radius: 0.5rem;
		border-bottom-right-radius: 0.5rem;
	}

	.files tr td:nth-last-of-type(1) {
